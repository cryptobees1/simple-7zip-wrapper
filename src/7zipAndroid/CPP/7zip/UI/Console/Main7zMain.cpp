...
bool R7zipMain::_Main1(const UStringVector& commandStrings)
{
   //NT_CHECK

   bool bRet = false;
   try {
      bRet = _Main2(commandStrings);
   }
   catch (...) {
      return false;
   }
...
bool R7zipMain::_Main2(const UStringVector& commandStrings)
{
  ...
  if (codecs->Load() != S_OK) return false;
        ...
        || options.Command.IsFromUpdateGroup()))
  {
    ...
    throw "7-Zip cannot find the code that works with archives.";
  }
  ...
  */
  ...
  if (isExtractGroupCommand || options.Command.CommandType == NCommandType::kList)
  {
    ...
    if (isExtractGroupCommand)
    {
      ...
      if (!options.HashMethods.IsEmpty())
      {
        ...
        if (hb.SetMethods(EXTERNAL_CODECS_VARS_L options.HashMethods) != S_OK) return false;
        hb.Init();
      }
  ...
  else if (options.Command.IsFromUpdateGroup())
  {
    ...
    openCallback.Init(NULL, NULL, NULL);
...
bool R7zipMain::_IsValid7zArchiveHeaderFormat(const UString& sInArchiveFullPath)
{
#if defined(_WIN32) && !defined(UNDER_CE)
  SetFileApisToOEM();
#endif
  ...
  // Find the file
  NFind::CFileInfo fi;
  if (!fi.Find(us2fs(sInArchiveFullPath)) || fi.IsDir()) return false;

  // Open the file stream
  CInFileStream* fileStreamSpec = new CInFileStream;
  if (!fileStreamSpec->Open(us2fs(sInArchiveFullPath))) {
    delete fileStreamSpec;
    return false;
  }

  // Go to beginning of file stream
  UInt64 newPosition;
  if (!fileStreamSpec->File.Seek(0, newPosition)) {
    delete fileStreamSpec;
    return false;
  }
  ...
  const size_t kBufSize = 6;
  byteBuffer.Alloc(kBufSize);
  UInt32 processedSize;
  HRESULT res = fileStreamSpec->Read(byteBuffer, kBufSize, &processedSize);
  if ((res != S_OK) || (processedSize != kBufSize)) {
    delete fileStreamSpec;
    return false;
  }

  // Match the header
  const Byte k7zipHeader[] = { 0x37, 0x7a, 0xbc, 0xaf, 0x27, 0x1c };
  const Byte *buf = byteBuffer;
  for (size_t i = 0; i < kBufSize; i++) {
    if (buf[i] != k7zipHeader[i]) {
      delete fileStreamSpec;
      return false;
    }
  }
